...
wearing red or blue shirts. In fact, exactly half of the class is wearing red shirts, 
and the other half is wearing blue shirts. You're responsible for arranging the students in two rows before taking the photo.
...
and each height will be a positive integer. Write a function that returns whether or not a class photo 
...
def classPhotos(redShirtHeights:List[int], blueShirtHeights:List[int]):
    redShirtHeights.sort() #nlogn
    blueShirtHeights.sort() # nlogn
    front_rows = blueShirtHeights if redShirtHeights[-1] > blueShirtHeights[-1] else redShirtHeights
    back_rows = redShirtHeights if id(blueShirtHeights) == id(front_rows) else blueShirtHeights
    return all(front_rows[i] < back_rows[i] for i in range(len(front_rows)))
