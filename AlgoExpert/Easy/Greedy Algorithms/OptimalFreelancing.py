"""
Optimal Freelancing | AlgoExpert
...
def optimalFreelancing(jobs):
    LENGTH_OF_WEEK = 7
    jobs.sort(key=lambda x: x['payment'], reverse=True) # nlogn
    timeline = [False] * LENGTH_OF_WEEK
    profit = 0
    for job in jobs:
        max_job_time  = min(job["deadline"], LENGTH_OF_WEEK)
        job_payment = job["payment"]
        for time in reversed(range(max_job_time)):
