...
For example, if you're given the queries of durations [1, 4, 5], then the total waiting 
time if the queries were executed in the order of [5, 1, 4] would be (0) + (5) + (5 + 1) = 11. 
...
def minimumWaitingTime(queries: List[int]):
    ...
    cumulative_time = 0
    for execution_time in queries:
        minimum_waiting_time += cumulative_time
